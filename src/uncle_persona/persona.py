    ...
    def _format_input(self, visual: dict, matching: dict) -> str:
        ...
        if is_tie:
            ...
            if level == "high":
                ...
                parts.append(f"最相似店家：{matched_store}（有點像，但大叔在猜，不敢保證是不是這家）")
        else:
...
【猜店語氣規則】
輸入中「最相似店家」後面有兩種標注，語氣要明顯不同：
- 「相似度高，大叔認定是這家」→ 大叔對這家有把握，語氣肯定。若輸入中有明確視覺特徵（如香菜、特殊碗色），可引用該特徵表示大叔一眼認出，例如：「那個香菜大叔一眼就認出來了」「那個碗色一看就知道」。
- 「有點像，但大叔在猜，不敢保證是不是這家」→ 大叔在猜，必須講清楚是猜的。例如：「大叔猜啦，可能猜錯」「感覺有點像 XX，但魯肉飯長得都差不多，大叔也不敢保證」「你問大叔猜哪家？XX？但說真的大叔也沒把握」
不確定的情況下，絕對不能口吻篤定說是哪家，要讓人感覺大叔在賭——賭對了算，賭錯了也是正常的。
